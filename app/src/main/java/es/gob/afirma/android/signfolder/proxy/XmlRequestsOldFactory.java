...
final class XmlRequestsOldFactory {
	 ...
	 * @param certEncoded Certificado del solicitante codificado en Base64
	 * @param request Petici&oacute;n de firma
	 * @return Solicitud de prefirma a partir de una lista de peticiones de firma */
	static String createPresignRequest(final String certEncoded, final SignRequest request){
		if (request == null) {
	...
	static String createPostsignRequest(final String certEncoded, final TriphaseRequest[] requests) {
		if (requests == null) {
			throw new IllegalArgumentException("La lista de peticiones no puede ser nula"); //$NON-NLS-1$
		}
		if (certEncoded == null) {
	...
	/**
	 * Crea el XML para la solicitud de rechazo de peticiones.
	 * @param certEncoded Certificado del solicitante codificado en Base64.
	 * @param requestIds Lista de identificadores de las peticiones de firma a rechazar.
     * @param reason Motivo del rechazo.
	 * @return XML de la solicitud de rechazo. */
	static String createRejectRequest(final String certEncoded, final String[] requestIds, final String reason) {
	...
	static String createPreviewRequest(final String certEncoded, final String documentId) {
	...
	static String createApproveRequest(final String certEncoded, final String[] requestIds) {
		if (requestIds == null || requestIds.length == 0) {
			throw new IllegalArgumentException("La lista de peticiones no puede ser nula"); //$NON-NLS-1$
		}
