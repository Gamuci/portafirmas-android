...
final class RequestSigner implements PrivateKeySelectionListener {
	 ...
	 */
	RequestSigner(final String certAlias, final OperationRequestListener operationListener, final Context context) {
	...
	private void doSign(final SignRequest[] reqs, final PrivateKey pk, final X509Certificate[] certChain) {
		for (final SignRequest req : reqs) {
			new SignRequestTask(req, pk, certChain, CommManager.getProxyConnector(), this.opListener).execute();
		}
	}
