...
/** Analizador de XML para la generaci&oacute;n de una verificaci&oacute;n de login con clave.
 * @author Sergio Mart&iacute;nez. */
public class ClaveLoginValidationResponseParser {

	private static final String CLAVE_LOGIN_VALIDATION_RESPONSE_NODE = "claveLoginValidation"; //$NON-NLS-1$
	...
	static RequestResult parse(final Document doc) {
		...
		if (!CLAVE_LOGIN_VALIDATION_RESPONSE_NODE.equalsIgnoreCase(doc.getDocumentElement().getNodeName())) {
			throw new IllegalArgumentException("El elemento raiz del XML debe ser '" + //$NON-NLS-1$
					CLAVE_LOGIN_VALIDATION_RESPONSE_NODE + "' y aparece: " + //$NON-NLS-1$
	...
	private static final class RequestResultParser {
		...
		private static final String CLAVE_LOGIN_RESPONSE_NODE = "claveLoginValidation"; //$NON-NLS-1$
