...
final class XmlUtils {
	...
	/**
	 * Recupera el contenido de un nodo. Se utiliza este metodo en lugar del
	 ...
	 * @return Texto contenido en el nodo o cadena vaci&iacute;a.
	 */
	static String getTextContent(final Node node) {
		Node child;
		StringBuilder sContent = new StringBuilder();
		sContent.append(node.getNodeValue() != null ? node.getNodeValue() : ""); //$NON-NLS-1$
		final NodeList nodes = node.getChildNodes();
		for(int i = 0; i < nodes.getLength(); i++) {
			child = nodes.item(i);
			sContent.append(child.getNodeValue() != null ? child.getNodeValue() : ""); //$NON-NLS-1$
			if(nodes.item(i).getChildNodes().getLength() > 0) {
				sContent.append(getTextContent(nodes.item(i)));
			}
		}
		return sContent.toString().trim();
	}
}
