...
public final class PresignsResponseParser {
	...
	static TriphaseRequest[] parse(final Document doc) {
		...
		if (!PRESIGN_RESPONSE_NODE.equalsIgnoreCase(docElement.getNodeName())) {
	...
	/**
	 * Analiza un elemento XML y, en caso de tener el formato correcto, obtiene de &eacute;l
	 * un listado de objetos {@link TriphaseRequest}.
	 * @param element Nodo XML.
	 * @return Objeto con los datos del XML.
	 * @throws IllegalArgumentException Cuando el XML no tiene el formato esperado.
	 */
	static TriphaseRequest[] parse(final Element element) {

		if (element == null) {
			throw new IllegalArgumentException("El nodo XML proporcionado no puede ser nulo");  //$NON-NLS-1$
		}

		final Vector<TriphaseRequest> listPresignRequests = new Vector<TriphaseRequest>();
		final NodeList requestNodes = element.getChildNodes();
